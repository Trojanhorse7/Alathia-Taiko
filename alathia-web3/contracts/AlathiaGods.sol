...
contract AlathiaGods is ERC1155, Ownable, ERC1155Supply {
  ...
  struct Player {
    ...
    uint256 gamesPlayed; /// @param gamesPlayed total number of games played by the player
    uint256 gamesWon; /// @param gamesWon total number of games won by the player
    uint256 gamesLost; /// @param gamesLost total number of games lost by the player
  }
  ...
  function initialize() private {
    ...
    players.push(Player(address(0), "", 0, 0, false, 0, 0, 0));
  ...
  function registerPlayer(string memory _name, string memory _gameTokenName) external {
    ...
    players.push(Player(msg.sender, _name, 10, 25, false, 0, 0, 0)); // Initialize stats
  ...
  /// @dev This function would be updated later to a true source of randomness. - Trojan
  ...
  function _resolveBattle(Battle memory _battle) internal {
    ...
    if (p1.move == 1 && p2.move == 1) {
      if (p1.attack >= p2.health) {
        ...
        // Both player's health are damaged
    ...
    // Reset player moves to 0
    _battle.moves[0] = 0;
    _battle.moves[1] = 0;
    updateBattle(_battle.name, _battle);

    // Reset player attack and defense strength
  ...
  function _endBattle(address battleEnder, Battle memory _battle) internal returns (Battle memory) {
    ...
    players[playerInfo[battleEnder]].gamesPlayed++;
    players[playerInfo[_battleLoser]].gamesPlayed++;

    // Increment games won and games lost for both players
    players[playerInfo[battleEnder]].gamesWon++;
    players[playerInfo[_battleLoser]].gamesLost++;
  ...
  // Function to get leaderboard function
  function getLeaderboard(uint256 limit) public view returns (Player[] memory) {
    Player[] memory sortedPlayers = players;

    // Sort players by gamesWon in descending order
    for (uint256 i = 0; i < sortedPlayers.length - 1; i++) {
      for (uint256 j = i + 1; j < sortedPlayers.length; j++) {
        if (sortedPlayers[i].gamesWon < sortedPlayers[j].gamesWon) {
    ...
    // Return limited number of top players
    uint256 returnLength = limit > sortedPlayers.length ? sortedPlayers.length : limit;
    Player[] memory leaderboard = new Player[](returnLength);
    for (uint256 i = 0; i < returnLength; i++) {
      leaderboard[i] = sortedPlayers[i];
    }

    return leaderboard;
  }

  // Function to turn uint256 into string
